	...
	const authenticateHandler = () => {
		...
		if (basicValidation.valid) {
			...
			axios
				...
				.then(res => {
					const data = res.data;
					axios.get(`${config.databaseURL}/users/${data.localId}.json`).then(userRes => {
						const userData = userRes.data;
						if (userData.roles && userData.roles.find(user => user === 'admin')) {
	...
	return (
		<>
			...
			<Grid className={classes.containerSpace} container justify="center">
				<Grid item>
					<img src={LogoOrange} alt="neurobody logo" />
				</Grid>
				<Grid className={classes.containerSpace} container justify="center">
