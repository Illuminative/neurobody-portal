...
const ExerciseDetail = props => {
	...
	useEffect(() => {
		...
		if (tag !== '') {
			axios.get(`${config.databaseURL}/exercises/${tag}.json`).then(res => {
				...
				if (res.data.engages.primary) setEngagePrimary(res.data.engages.primary);
				if (res.data.engages.secondary) setEngageSecondary(res.data.engages.secondary);
				if (res.data.engages.isometric) setEngageIsometric(res.data.engages.isometric);
	...
	const onSaveExercisehandler = () => {
		...
		axios.put(`${config.databaseURL}/exercises/${exerciseTag}.json`, exercise).then(res => {
			console.log(res);
	...
	const onDeleteExerciseHandler = () => {
		axios.delete(`${config.databaseURL}/exercises/${exerciseTag}.json`).then(res => {
			console.log(res);
			props.history.push(process.env.PUBLIC_URL + '/exercises');
		});
	};
